class Node {
    ...
    printLeafNodes(root , arr){
        if(root === null){
            ...
            if(root.left == null && root.right == null){
                ...
                this.printLeafNodes(root.left , arr);
                this.printLeafNodes(root.right , arr);
            }
        }
    }

    printLeftNodesValue(root){
        if(root === null){
            return null;
        }else{
            if(root.left !== null){
                console.log(root.left.value);
                this.printLeftNodesValue(root.left);
                this.printLeftNodesValue(root.right);       
            }
        }
    }
  ...
  */
  ...
  bt.insert(12);
  let output = [];
  bt.printLeafNodes(bt.root , output);
  console.log(output);
  bt.printLeftNodesValue(bt.root);